    ...
    try {
      ...
      if (getUser) {
        ...
        const seller = await sellerModel.create({
          name,
          email,
          password: await bcrpty.hash(password, 10), // Hash the password before saving
          method: "manual", // Default method set to 'manual'
          shopInfo: {}, // Default shopInfo set to an empty object
        });
        console.log(seller);
      }
    } catch (error) {
